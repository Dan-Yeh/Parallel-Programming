...
// Struct to hold some stuff for the thread function
struct stuff {
    int tid;
    Camera* camera;
...
void* do_work(void* args) {
    stuff t_stuff = *(stuff*) args;
    int tid = t_stuff.tid;
    // Pixels per thread
    int thread_pixels = (HEIGHT * WIDTH + NUM_THREADS) / NUM_THREADS;
    // Calculate local checksum per thread and add up afterwards
    Vector3* local_checksum = new Vector3(0,0,0);

    // i < HEIGHT*WIDTH to avoid overcount
    for (int i = tid*thread_pixels; i < (tid+1)*thread_pixels && i < HEIGHT*WIDTH; ++i) {
        int y = i/WIDTH;
        int x = i%WIDTH;

        Vector3 pixel_color(0,0,0);
        for(int s = 0; s < SAMPLES; s++) {
            auto u = (float) (x + rnd) / (WIDTH - 1);
            auto v = (float) (y + rnd) / (HEIGHT - 1);
            auto r = get_camera_ray(*t_stuff.camera, u, v);
            pixel_color += trace_ray(r, *t_stuff.spheres, DEPTH);
        }
        auto output_color = write_color(*local_checksum, pixel_color, SAMPLES);

        int pos = ((HEIGHT - 1 - y) * WIDTH + x) * 3;
        t_stuff.image_data[pos] = output_color.r;
        t_stuff.image_data[pos + 1] = output_color.g;
        t_stuff.image_data[pos + 2] = output_color.b;
    }
    
    return (void*) local_checksum;
}
...
int main(int argc, char **argv) {
    ...
    stuff buffer[NUM_THREADS];
    float rays[WIDTH*HEIGHT*SAMPLES];
    for (size_t y=HEIGHT - 1; y<=0; --y) {
        for (size_t x=0; x<WIDTH; ++x) {
            for (size_t s=0; s<SAMPLES; ++s) {
                auto u = (float) (x + random_float()) / (WIDTH - 1);
                auto v = (float) (y + random_float()) / (HEIGHT - 1);
                rays[] = get_camera_ray(camera, u, v);          
            }
        }
    }
    ...
    for (int tid = 0; tid < NUM_THREADS; ++tid) {
        buffer[tid].tid = tid;
        buffer[tid].camera = &camera;
        buffer[tid].spheres = &spheres;
        buffer[tid].image_data = image_data;
        pthread_create(&threads[tid], NULL, do_work, &buffer[tid]);
    }
    for (int tid = 0; tid < NUM_THREADS; ++tid) {
        void* local_checksum;
        pthread_join(threads[tid], &local_checksum);
        checksum += *(Vector3*) local_checksum;
        free(local_checksum);
    } 
